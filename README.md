...
two options for now:
  ...
  - Take words in a sentence and merge the same words sung by different singers. Output will be a wav audio file consisting     of the actual words with music.
  - Take the whole sentence and output the most similar lyrics sung a by a singer. Output will be the same as first step

## Process followed
### Words to Music
  - An opernsource [dataset](https://github.com/mattmcvicar/lyric_database/tree/master/aligned) was used which has aligned     words for each song though iit is not very accurate.
